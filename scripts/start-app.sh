#!/usr/bin/env bash
# Arranca backend (:4001) y frontend (:5174) en paralelo.
...
# ---- Arranque -------------------------------------------------------------
echo "============================================================"
echo "  my-finanzas  →  arrancando servers"
echo "============================================================"
...
if [ "$OPEN_BROWSER" = true ]; then
  (
    ...
    until curl -s http://localhost:4001/health > /dev/null 2>&1 \
       && curl -s http://localhost:5174        > /dev/null 2>&1; do
      sleep 0.3
    done
    # En WSL, abrimos el navegador del HOST Windows
    if command -v wslview > /dev/null 2>&1; then
      wslview http://localhost:5174
    elif command -v explorer.exe > /dev/null 2>&1; then
      explorer.exe http://localhost:5174 || true
